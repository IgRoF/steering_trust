...
> **➡️ Latest work (14 June 2026):** The first public version of my **[MASK Benchmark Replication](mask-benchmark-replication/)** package is available in this repo, and the next run of local MASK experiments (with newer and reasoning models) is mostly complete. The next update is mostly publication work. If you are here to look at the released public package, start with [mask-benchmark-replication/README.md](mask-benchmark-replication/README.md); the shorter current-results entry point is [mask-benchmark-replication/docs/results/results-overview.md](mask-benchmark-replication/docs/results/results-overview.md).
...
1. **Evaluating honesty at the frontier:** Replicate the MASK benchmark on the latest frontier models to establish baselines and test whether the original finding (that honesty does not improve with scale) persists. *(first public version released; extended experiments mostly completed locally, public refresh pending)*
...
**Status (14 June 2026):** The first public version of the replication project is still under [`mask-benchmark-replication/`](mask-benchmark-replication/). Since that release from April, I've started a second experimental phase with newer and reasoning model; the remaining work is running some more experiments and re-analysing whether the trends from the first run (see below) still hold.
 
**Brief summary of findings (only for the completed first run):**
